...
import sys
sys.path.append(
    "/data/ephemeral/home/nlp-5/eunbyul/ah"
)
...
def inference(config, generate_model, tokenizer, config_path):
    ...
    # config_path에서 파일명만 뽑기
    config_filename = os.path.splitext(os.path.basename(config_path))[0]

    # 현재 시간
    now_str = datetime.now().strftime("%Y%m%d_%H%M%S")

    save_filename = f"result_{config_filename}_{now_str}.csv"
    save_path = os.path.join(config['inference']['result_path'], save_filename)
