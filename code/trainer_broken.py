...
class DialogueSummarizationTrainer:
    ...
    def initialize_components(self) -> None:
        ...
        # 데이터 프로세서 초기화 (모델 객체 전달)
        self.data_processor = DataProcessor(
            tokenizer=self.tokenizer,
            config=self.config,
            model=self.model  # 모델 객체 전달
        )
    ...
    def _load_model_standard(self, model_checkpoint: str, architecture: str) -> None:
        ...
        # 특수 토큰과 모델 embedding 호환성 검사 및 조정
        try:
            from utils.model_utils import resize_model_embeddings, validate_tokenizer_model_compatibility
            
            # 호환성 검사
            if not validate_tokenizer_model_compatibility(self.tokenizer, self.model):
                logger.warning("토크나이저와 모델의 vocab size가 불일치합니다. 자동 조정합니다...")
                self.model = resize_model_embeddings(self.model, self.tokenizer)
        except ImportError:
            logger.warning("⚠️ model_utils를 찾을 수 없습니다. embedding 크기 검사를 건너뜁니다.")
    ...
    def _save_results(self, result: TrainingResult) -> None:
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            if result.wandb_run_id:
                ...
                if hasattr(result, 'submission_file') and result.submission_file:
