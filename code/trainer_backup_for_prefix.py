...
class NMTTrainer:
    ...
    def _get_model_specific_config(self, model_name: str) -> Dict[str, Any]:
        ...
        def _preprocess_for_model(self, text: str, model_type: str = None) -> str:
            ...
            # 모델 타입에 따른 전처리
            if model_type == 't5':
                # T5/FLAN-T5: config에서 prefix 읽어오기 또는 기본값 사용
                prefix = self._get_t5_prefix(model_name)
                processed_text = f"{prefix}{original_text}"
                logger.debug(f"T5 preprocessing with prefix '{prefix}': '{processed_text}'")
                ...
                def _get_t5_prefix(self, model_name: str) -> str:
                """
                T5 모델에 대한 prefix 반환
                
                config에서 input_prefix를 읽어오거나, 모델별 기본값 사용
                ...
                try:
                # 모델별 설정에서 prefix 찾기
                model_specific_config = self._get_model_specific_config(model_name)
                
                # general 섹션에서 input_prefix 찾기
                general_config = model_specific_config.get('general', {})
                if 'input_prefix' in general_config:
                    prefix = general_config['input_prefix']
                    logger.debug(f"Using config-specified prefix for {model_name}: '{prefix}'")
                    return prefix
                    
                except Exception as e:
                logger.debug(f"Failed to get model-specific config for {model_name}: {e}")
                
                # 모델명에 따른 기본 prefix 결정
                model_name_lower = model_name.lower()
                
                # eenzeenee 모델은 항상 'summarize: ' prefix 사용
                if 'eenzeenee' in model_name_lower:
                return "summarize: "
                
                # 다른 T5 모델들에 대한 기본 prefix
                elif any(keyword in model_name_lower for keyword in ['mt5', 'multilingual']):
                return "summarize: "  # mT5도 summarize prefix 사용
